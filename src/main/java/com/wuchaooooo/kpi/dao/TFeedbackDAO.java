...
public interface TFeedbackDAO {
    ...
    List<PFeedback> getFeedbacksByStudentId(@Param("studentId") long studentId);

    @Select("select * from " + TABLE_NAME + " where `id` = #{feedbackId}")
    PFeedback getFeedbackById(@Param("feedbackId") long feedbackId);
    ...
    long insertFeedback(PFeedback pFeedback);
    ...
    void removeFeedBack(@Param("id") long feedBackId);
}
